...
int main(){
	...
	size_t len;
	len = sizeof(size_t);
	int a = sysctlbyname("kern.sched.kernChoice", &choice, &len, NULL, 0);
	printf("kern.sched.kernChoice = %d\n", a);
	...
	if(child1 == 0){
	  ...
	  getrusage(RUSAGE_SELF, &time_buff1);
		...
		printf("System CPU time used: %1ld.%06ld seconds\n",  time_buff1.ru_stime.tv_sec, time_buff1.ru_stime.tv_usec);
		...
		if(child2 == 0){
			...
			printf("System CPU time used: %1ld.%06ld seconds\n",  time_buff2.ru_stime.tv_sec, time_buff2.ru_stime.tv_usec);
			...
			if(child3 == 0){
				...
				printf("System CPU time used: %1ld.%06ld seconds\n",  time_buff3.ru_stime.tv_sec, time_buff3.ru_stime.tv_usec);
