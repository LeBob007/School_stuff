      ...
      while (!cmdDone) {
        ...
        for (argc = nxtCmdIndex; argc < i; argc++) {
          ...
          // Check for 'cd' command
            ...
            if (args[argc + 1] ==
                NULL) { // If just 'cd' then go to HOME variable
              char *dir = getenv("HOME");
              chdir(dir);
            } else { // Else 'cd' to the next arg
              chdir(args[argc + 1]);
            }
          }
          // Check for ';'
        ...
        if (pid == 0) {
          ...
          if (execvp(args[currIndex], &args[currIndex]) == -1) {
            perror("execvp() error\n");
            exit(1);
          }
        } else {
